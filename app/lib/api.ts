...
export async function updateCashierApi(data: {
  ...
  if (res.status !== "200") throw new Error(res.message || "Failed to update cashier");
}

export async function deleteCashierApi(id: string): Promise<void> {
  const res = await post<unknown>("/cashiers/delete", { id });
  if (res.status !== "200") throw new Error(res.message || "Failed to delete cashier");
}
...
export async function updatePlayerApi(data: {
  ...
  if (res.status !== "200") throw new Error(res.message || "Failed to update player");
}
...
export async function updateTransactionApi(data: {
  ...
  if (res.status !== "200") throw new Error(res.message || "Failed to update transaction");
}
...
export async function lockTransactionApi(transactionId: string): Promise<void> {
  ...
  if (res.status !== "200") throw new Error(res.message || "Failed to lock transaction");
}

// ─── Daily Report ──────────────────────────────────────

export async function getDailyReport(businessDate: string): Promise<ApiDailyReport | null> {
  const res = await post<ApiDailyReport>("/GetDailyReportByBusinessDate", { businessDate });
...
function mapCashier(api: ApiCashier): Cashier {
  ...
  const role: Role = validRoles.includes(api.role as Role) ? (api.role as Role) : "cashier";
  return {
    id: api.id,
...
export function mapApiTransaction(api: ApiTransaction): Transaction {
  return {
    id: api.id,
    direction: (api.direction === "outgoing" ? "outgoing" : "incoming") as Direction,
...
export function mapApiPlayer(api: ApiPlayer, transactions: Transaction[] = []): Player {
  return {
    id: api.id,
